...
from ev3dev2.motor import OUTPUT_B, OUTPUT_C, DcMotor, MediumMotor
...
class MindstormsGadget(AlexaGadget):
    ...
    def __init__(self):
        ...
        self.leds = Leds()
        self.sound = Sound()
        self.cup_motor = MediumMotor(OUTPUT_B)
        self.track_motor = DcMotor(OUTPUT_C)
        self.ir_link = Sensor(INPUT_1)
    ...
    def on_alexa_gadget_musicdata_tempo(self, directive):
        ...
        for tempo in tempo_data:
            ...
            if tempo.value > 0:
                # dance pose
                self.cup_motor.run_timed(speed_sp=750, time_sp=2500)
                self.track_motor.run_timed(duty_cycle_sp=100, time_sp=150)
                self.leds.set_color("LEFT", "GREEN")
                self.leds.set_color("RIGHT", "GREEN")
                time.sleep(3)
    ...
    def _dance_loop(self, bpm):
        ...
        while self.trigger_bpm == "on":
            ...
            cup_motor_speed = -cup_motor_speed
            track_duty_cycle = -track_duty_cycle

            self.leds.set_color("LEFT", led_color)
            self.leds.set_color("RIGHT", led_color)
            self.cup_motor.run_timed(speed_sp=cup_motor_speed, time_sp=150)
            self.track_motor.run_timed(duty_cycle_sp=track_duty_cycle, time_sp=150)
